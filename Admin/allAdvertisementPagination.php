...
//continue only if $_POST is set and it is a Ajax request
    if (isset($_GET) && isset($_SERVER['HTTP_X_REQUESTED_WITH']) && strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest') {
        ...
        if ($query === "=====+++=====") {
            if ($dastebandi == 'all') {
                if ($type == 1)
                    $results = $connection->query("SELECT COUNT(*) FROM advertisement ORDER BY startTime DESC ");
            } else {
//                if ($type == 1)
//                    $results = $connection->query("SELECT COUNT(*) FROM advertisement WHERE dastebandi='$dastebandi'  ORDER BY startTime DESC");
            }
        } else {
            $D = 0;
            $command = "WHERE";
...
//                        $results = $connection->query("SELECT COUNT(*) FROM advertisement WHERE dastebandi='$dastebandi'  ORDER BY startTime DESC");
                ...
                if ($dastebandi == 'all') {
                    if ($type == 1)
                        $results = $connection->query("SELECT COUNT(*) FROM advertisement " . $command." ORDER BY startTime DESC");
                } else {
//                    if ($type == 1)
//                        $results = $connection->query("SELECT COUNT(*) FROM advertisement " . $command . " and dastebandi='$dastebandi'  ORDER BY startTime DESC");
                }
            }
        }
        ...
        if ($query === "=====+++=====") {
            ...
            if ($dastebandi == 'all') {
                if ($type == 1)
                    $results = $connection->query("SELECT * FROM advertisement ORDER BY startTime DESC  LIMIT $page_position, $item_per_page  ");
            } else {
//                if ($type == 1)
//                    $results = $connection->query("SELECT * FROM advertisement WHERE dastebandi='$dastebandi' ORDER BY startTime DESC  LIMIT $page_position, $item_per_page ");
            }
        } else {
            $command = "WHERE";
            $command2 = "WHERE";
...
//                        $results = $connection->query("SELECT * FROM advertisement  WHERE dastebandi='$dastebandi' ORDER BY startTime DESC  LIMIT $page_position, $item_per_page ");
                ...
                if ($dastebandi == 'all') {
                    if ($type == 1)
                        $results = $connection->query("SELECT * FROM advertisement " . $command . " ORDER BY startTime DESC LIMIT $page_position, $item_per_page ");
                } else {
//                    if ($type == 1)
//                        $results = $connection->query("SELECT * FROM advertisement " . $command . "and dastebandi='$dastebandi'" . " ORDER BY startTime DESC LIMIT $page_position, $item_per_page ");
                }
            }
        }
        ...
        if ($type == 1)
            echo '<table class="table user-list text-right">
                                ...
                                <tr>
                                    ...
                                    <th><span>وضعیت نمایش</span></th> 
                                    <th><span>وضعیت پرداخت</span></th> 
                                    <th><span>نوع آگهی</span></th>
                                    <th><span>متن آگهی</span></th>                                   
                                    <th><span>زمان پایان</span></th>                                   
                                </tr>
                                </thead>';
        echo '<tbody>';
...
//echo $row['name'];
            ...
            if ($type == 1) {
                ...
                echo "
                                        <a  href='allAdvertisement.php' class=\"" . $row['ID'] . "\" id='changestatusOk'>
                                            <span class=\"fa-stack\">
                                        ...
                                        <a  href='allAdvertisement.php' class=\"" . $row['ID'] . "\" id='changestatusNO'>
                                            <span class=\"fa-stack\">
            ...
            echo "                                                                      
                                    <td style=\"width: 20%;\">
                                        <a  target='_blank' href='addAdvertisement.php?type=$type&product=" . $row['ID'] . "' class=\"table-link\">
                                            <span class=\"fa-stack\">
                                                <i class=\"fa fa-square fa-stack-2x bluecolor\"></i>
                                                <i class=\"fas fa-edit fa-stack-1x fa-inverse \"></i>
                                            </span>
                                        </a>
                                        <a onClick=\"return confirming();\"  href='deleteblog.php?type=10&product=" . $row['ID'] . "' class=\"table-link danger\">
                                            <span class=\"fa-stack\">
