...
$(document).ready(function() {
    ...
    $.get("allPardakhtPagination.php", {page:1, limit:range,type:type,query:"=====+++=====",category:"all"}, function (data, status) {
        $("#results").html(data);
    });
    ...
    $("#results").on( "click", ".pagination a", function (e){
        ...
        $.get("allPardakhtPagination.php", {page:page, limit:range,type:type,query:"=====+++=====",category:"all"}, function (data, status) {
    ...
    $('#sarching').bind("enterKey",function(e){
        ...
        $(".loading-div").show(); //show loading element
        range = $("#limit").find(":selected").text();
        $.get("allPardakhtPagination.php", {page:1, limit:range,type:type,query:searchdata,category:"all"}, function (data, status) {
            $(".loading-div").hide(); //show loading element
            $("#results").html(data);
        });
    });
    ...
    $(document).on('change','.status',function(e){
        ...
        if (confirm('از تغییر استیت این کامنت مطمعنید؟')){
            $.get("changestatus.php", {orderID:orderID, status:status , typ:1}, function (data, status) {
                if (data=="0"){
